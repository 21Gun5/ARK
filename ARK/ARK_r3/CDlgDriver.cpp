...
BEGIN_MESSAGE_MAP(CDlgDriver, CDialogEx)
	...
	ON_NOTIFY(NM_RCLICK, IDC_LIST1, &CDlgDriver::OnRclickList1)
	ON_COMMAND(ID_HIDEDRIVER, &CDlgDriver::OnHidedriver)
	//ON_BN_CLICKED(IDC_BUTTON_RENEW, &CDlgDriver::OnBnClickedButtonRenew)
	ON_COMMAND(ID_RENEW, &CDlgDriver::OnRenew)
END_MESSAGE_MAP()
...
void CDlgDriver::OnRclickList1(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CDlgDriver::OnHidedriver()
{
	...
	DWORD size = 0;
	DeviceIoControl(g_hDev, HideDriver, &index, sizeof(DWORD), NULL, 0, &size, NULL);
}

//void CDlgDriver::OnBnClickedButtonRenew()
//{
//	// TODO: 在此添加控件通知处理程序代码
//	
//	// 刷新前先清空所有
